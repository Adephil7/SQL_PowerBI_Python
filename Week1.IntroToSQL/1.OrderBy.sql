--ORDER by is used to SORT data by a particular column
...
--where sorting the spreadsheet by column actually alters the data in that sheet
--until you undo or change that sorting
...
In query #2, since you sorted by the total dollar amount first, the orders appear from greatest to
least regardless of which account ID they were from. Then they are sorted by account ID next.
(The secondary sorting by account ID is difficult to see here, since only if there were two orders
